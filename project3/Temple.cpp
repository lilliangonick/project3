...
Temple::Temple(Player* pointer, int level, int goblinSmellDistance) : player(pointer), m_level(level), stairs(new GameObject("stairs", "step")), idol(new GameObject("golden idol", "yay")), m_goblinSmellDistance(goblinSmellDistance) {
    int counter = 0;
    do {
        for (int i = 0; i < 18; i++) {
            for (int j = 0; j < 70; j++) {
...
bool Temple::allRoomsConnected() {
    ...
    // if number of islands is 1, then all the rooms are connected
...
// see if the move is valid (monsters can not move to where a monster is or where a wall it)
...
// do not want the stairs, idol, or objects to get overwritten by an actor's movement
void Temple::keepObjects(Actor *mover) {
    m_map[mover->getYPos()][mover->getXPos()] = ' ';
    if (mover->getXPos() == stairs->getXPos() && mover->getYPos() == stairs->getYPos()) {
        setStairsSpawn(mover->getXPos(), mover->getYPos());
    }
    if (mover->getXPos() == idol->getXPos() && mover->getYPos() == idol->getYPos()) {
        setIdolSpawn(mover->getXPos(), mover->getYPos());
    }
    
    if (isObjectAt(mover->getXPos(), mover->getYPos())) {
        for (vector<GameObject*>::iterator it = objects.begin(); it != objects.end(); ++it) {
            if ((*it)->getXPos() == mover->getXPos() && (*it)->getYPos() == mover->getYPos()) {
...
void Temple::movePlayer(char c) {
    ...
    // do not want stairs or objects to get overwritten
    keepObjects(player);
    ...
    for (vector<Monster*>::iterator it = monsters.begin(); it != monsters.end(); ++it) {
        if ((*it)->isSleeping()) {
            ...
            if ((*it)->smell(player)) {
                ...
                // vfs
                vector<array<int, 2>> path = bfs(*it, copy);
                
                // move the first move in the path (the start of the most optimal path)
                if (!path.empty()) {
                    for (size_t i = 1; i < path.size(); ++i) {
                        array<int, 2> nextPos = path[i];
                        if (validMonsterMove(nextPos[0], nextPos[1])) {
                            keepObjects(*it);
                            (*it)->setXPos(nextPos[0]);
                            (*it)->setYPos(nextPos[1]);
                            m_map[nextPos[1]][nextPos[0]] = 'G'; // reset
                            break; // only want one move!
                        }
                    }
                }
            }
...
bool Temple::isValidBFS(int x, int y, char copy[18][70]) {
    ...
    if (x < 0 || y < 0 || y >= 18 || x >= 70) {
        return false;
    }

    if (copy[y][x] == '#' || copy[y][x] == '1' || copy[y][x] == 'D' || copy[y][x] == 'S' || copy[y][x] == 'B') {
        return false;
    }
...
vector<array<int, 2>> Temple::bfs(Monster* goblin, char copy[18][70]) {
    ...
    copy[goblin->getXPos()][goblin->getYPos()] = '1';  // position is visited
    ...
    while (!q.empty()) {
        ...
        // if at player, return the path for the goblin to move
        if (x == player->getXPos() && y == player->getYPos()) {
            ...
            path.push_back({goblin->getXPos(), goblin->getYPos()});
            reverse(path.begin(), path.end());  // reverse path to get to player
            return path;
        }
        ...
        for (int i = 0; i < 4 ; i++) {
            ...
            if (isValidBFS(adjacentX, adjacentY, copy)) {
                ...
                copy[adjacentY][adjacentX] = '1';  // the position is visited now
                parent[adjacentY][adjacentX] = {x, y};  // save the cell for tracing
            }
        }
    }
    return path; // the path is empty, should not really ever come to this case
}
